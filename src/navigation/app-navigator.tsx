...
function AppTabs(){const { palette } = useTheme(); return <Tab.Navigator screenOptions={({route})=>({headerShown:false,tabBarActiveTintColor:palette.danger,tabBarInactiveTintColor:palette.muted,tabBarStyle:{height:64,paddingBottom:8,paddingTop:8,borderTopColor:palette.orange,borderTopWidth:2,backgroundColor:palette.white},tabBarIcon:({color,size})=><Ionicons name={({Map:'map-outline',Profile:'person-outline',Opportunities:'briefcase-outline',Settings:'settings-outline',Chat:'chatbubbles-outline',Events:'calendar-outline'} as const)[route.name]} size={size} color={color}/>})}>
